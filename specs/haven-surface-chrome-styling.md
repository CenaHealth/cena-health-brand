...
**Addendum 2026-07-16 (haven-pl-site 7b, R3 — linked eyebrow):** when the masthead eyebrow is the page's up-affordance (first instance: a PL-site component page's eyebrow linking to its category landing), the anchored variant (`document-masthead-eyebrow-link`, composed with `document-eyebrow`) keeps this register **unchanged at rest** — no underline, no color shift; the Overline stays quiet and does not announce itself as a control. Hover carries the affordance per the family's link-hover verdict: color shifts to `var(--color-sand-900)` + underline appears (`text-underline-offset: 4px`, `text-decoration-thickness: 1.5px`, `text-decoration-color: var(--color-sand-300)`); no background-fill. Focus-visible ring per §4.1. Same restraint rationale as the §2.3 mark-as-link addendum: link semantics ride the existing register rather than earning a new one. Use only when the eyebrow's level has a real landing page; the static eyebrow otherwise (a linked eyebrow to nowhere is a dead-end affordance).
