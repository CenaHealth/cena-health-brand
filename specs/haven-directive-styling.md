...
- **Rationale.**
  ...
  - **Why HTML controls stay in their current sand-bordered shape but earn a discipline note.** The shipped pan/zoom controls (bottom-right) are functional but read as widgets imported from outside the brand: white bg, sand-borders, generic mono labels. The brand-correct treatment is to push them toward the **secondary-button register** already in haven-ui (`sand-50` bg, `sand-300` border, sand-700 text on idle; teal-700 border on hover — the existing `btn-secondary` pattern). The buttons recede unless interactive. The zoom indicator (bottom-left) and help text (top-left) should drop their white-with-alpha bg in favor of `sand-50` at 92% opacity, matching the warm-ground rather than punching a cool hole in it. **Tier 1 PL promotion candidate** — controls treatment is a candidate to graduate to `pattern-library/components/diagram-viewport.html` so it inherits future haven button discipline automatically.
