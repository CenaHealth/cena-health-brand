...
### 3.5 Card variant — `.section-danger-zone-card`

Codified 2026-06-18 alongside the bare variant after the 2026-06-18 read of the rendered programs-edit screen surfaced that the bare top-bordered shape reads as floating when the surrounding page composes card-titled blocks (Summary, Details). The card variant wraps the same bound shape (title + description + action slot) in a card surface so it reads as belonging to the page's card rhythm without losing the quiet danger-zone register.

- **Semantic intent.** Same as the bare variant — a rarely-used destructive action with explanatory context — but rendered as its own card so the surrounding page's card-titled blocks have a coherent visual rhythm with the terminal section.
- **Visual treatment.** `.section-danger-zone-card` adds `mt-12` to the consumed `.card` + `.card-body` shape (48px top margin separates it from the preceding card or section, same as the bare variant; the card boundary supplies the visual separation the bare variant's top border + pt-6 provided). Title + description reuse `.section-danger-zone-title` + `.section-danger-zone-description` for typography parity — sand-700 font-medium sm title, sand-600 sm description, mb-2 / mb-4 / max-w-2xl spacing rhythm preserved.
- **Rationale.** The card boundary + the card-body padding (p-4 md:p-5) supply the structural separation the bare variant got from its top border. mt-12 carries the "page terminal section" signaling the bare variant also uses. Reusing the title + description classes single-sources the type discipline so the bare and card variants stay visually identical in everything except container shape — the loud question (card vs. bare) is asked once at consumer choice, and never again at typography review.
- **When to pick which variant.** Pick the card variant when the surrounding page's primary content blocks are themselves cards with card-titles or card-headers, so the danger zone joins the existing card rhythm (programs-edit Summary card + Details card → End program card). Pick the bare variant when the preceding content is non-card prose, lists, or settings rows, so the top border anchors the terminal section without introducing a card-shape that would itself read as floating (patient-profile dropout, kitchen-detail deactivate, patient-app delete-account).
- **The action slot stays unchanged.** Both variants compose a `confirm-dialog-danger-outline` (or sibling) in the action slot. The card variant does NOT introduce a new card-footer placement for the action — the action sits inside the card-body below the description, preserving the bare variant's "title → description → action" reading rhythm. A card-footer placement would re-introduce visual weight the section-danger-zone shape exists to keep quiet.
